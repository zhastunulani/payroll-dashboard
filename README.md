...
Meta токені **сайттың өз бетінен** қосылады: «Юнит және таргет» → Meta панелі → «Кабинеттер және
токендер» → токенді қойып сақтау. Ол дерекқорда шифрланып сақталады (`SESSION_SECRET`-тен алынған
кілтпен), сондықтан жергілікті машинада да, сайтта да бірден жұмыс істейді — серверге кірудің қажеті
жоқ. Қаласаңыз, айнымалы арқылы да беруге болады:
...
Дерекқордағы токен айнымалыдан басым емес: екеуі де қолданылады, біріктіріледі.
