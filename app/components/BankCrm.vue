...
const rows = computed(() => [
  { source: "amoCRM", note: t("ұтылған (выигранные) сделкалар"), deals: crmOf("amocrm")?.deals ?? null, amount: crmOf("amocrm")?.amount ?? null },
  { source: "CRM Eduser", note: t("төлемдер"), deals: crmOf("crm_eduser")?.deals ?? null, amount: crmOf("crm_eduser")?.amount ?? null },
  { source: t("Банк выпискасы"), note: t("сатылымдар, валовой оборот"), deals: s.value.salesCount + s.value.settlementCount, amount: s.value.turnover },
  { source: t("Нақты шотқа түскені"), note: t("Halyk шот выпискасы"), deals: s.value.settlementCount || null, amount: s.value.credited },
]);
...
<template>
  <div class="bank-crm">
    <h2 class="bank-section-title">{{ t("CRM-мен салыстыру") }} · {{ projectName }}</h2>
    <p class="bank-warning"><TriangleAlert :size="15" /> {{ t("Сверка агрегатталған түрде жасалды. Жүйелер арасында ортақ сделка немесе төлем ID-і болмағандықтан, клиенттер бойынша нақты салыстыру мүмкін емес.") }}</p>

    <div class="analytics-columns">
      <section class="panel analytics-panel">
        <header><div><span class="eyebrow">{{ periodLabel(bank.period.value) }}</span><h2>{{ t("Банк, CRM және amoCRM") }}</h2></div></header>
        <table class="pnl-table detail">
          <thead><tr><th scope="col">{{ t("Дереккөз") }}</th><th scope="col">{{ t("Сделка / төлем") }}</th><th scope="col">{{ t("Сома") }}</th></tr></thead>
          <tbody>
        ...
        <p class="panel-footnote">{{ t("CRM сандарын әзірге қолмен енгізесіз. API қосылғанда олар автоматты толады.") }}</p>
      </section>
      <section class="panel analytics-panel">
        <header><div><span class="eyebrow">{{ t("CRM деректері") }}</span><h2>{{ t("Айдың сандарын енгізу") }}</h2></div></header>
        <form class="bank-crm-form" @submit.prevent="save">
          <fieldset><legend>amoCRM</legend>
            <label class="form-field"><span>{{ t("Ұтылған сделка саны") }}</span><input v-model.number="form.amoDeals" type="number" min="0" step="1"></label>
            <label class="form-field"><span>{{ t("Сомасы, ₸") }}</span><input v-model.number="form.amoAmount" type="number" min="0"></label>
          </fieldset>
          <fieldset><legend>CRM Eduser</legend>
            <label class="form-field"><span>{{ t("Төлем саны") }}</span><input v-model.number="form.crmDeals" type="number" min="0" step="1"></label>
            <label class="form-field"><span>{{ t("Сомасы, ₸") }}</span><input v-model.number="form.crmAmount" type="number" min="0"></label>
          </fieldset>
          <button class="button primary" :disabled="busy">{{ busy ? t("Сақталуда…") : t("Сақтау") }}</button>
          <p v-if="error" class="finance-error" role="alert">{{ error }}</p>
        </form>
        <h3 class="bank-subhead">{{ t("Операциялар сверка күйі бойынша") }}</h3>
        <ul class="bank-recon-list"><li v-for="[status, n] in reconCounts" :key="status"><span>{{ t(RECON_LABELS[status]) }}</span><b>{{ n }}</b></li><li v-if="!reconCounts.length"><span>{{ t("Операция жоқ") }}</span></li></ul>
      </section>
    </div>
  </div>
</template>
