...
async function removeSelected() { if (selected.value.length && confirm(`${selected.value.length} төлем толық өшіріледі. Жалғастыру керек пе?`) && await payroll.mutate("deleteExpensesBulk", { ids: selected.value.map(item => item.id) })) closeSelection(); }
async function remove(expense: ExpenseRecord) { if (confirm(`${expense.name} өшірілсін бе?`)) await payroll.mutate("deleteExpense", { id: expense.id }); }
  ...
  <div v-if="payroll.data.value" class="page expenses-page">
    <section class="exp-summary">
      <button type="button" class="panel exp-card" :class="{ active: tab === 'mandatory' }" @click="selectTab('mandatory')">
        <header><span class="exp-icon mandatory"><ReceiptText :size="18" /></span>Міндетті төлемдер</header>
        ...
        <small v-if="unpaidCount" class="owed">Төленді {{ formatMoney(mandatoryPaid) }} · қалды {{ formatMoney(mandatoryTotal - mandatoryPaid) }} ({{ unpaidCount }} төлем)</small>
        <small v-else-if="mandatory.length" class="done">Барлығы төленді</small>
        <small v-else>Аренда, интернет, подписка — әлі енгізілмеген</small>
      </button>
      <button type="button" class="panel exp-card" :class="{ active: tab === 'other' }" @click="selectTab('other')">
        <header><span class="exp-icon other"><ShoppingBag :size="18" /></span>Басқа шығындар</header>
      ...
      <div class="panel exp-card static">
        <header><span class="exp-icon total"><WalletCards :size="18" /></span>Шығындар барлығы</header>
        <strong>{{ formatMoney(mandatoryTotal + otherTotal + targetTotal) }}</strong>
        <small>Айлықтан бөлек · {{ payroll.data.value.selectedMonth.label }}</small>
      </div>
    </section>

    <div class="segmented exp-tabs" role="tablist" aria-label="Шығын түрі">
      <button type="button" role="tab" :aria-selected="tab === 'mandatory'" :class="{ active: tab === 'mandatory' }" @click="selectTab('mandatory')">Міндетті төлемдер <b>{{ mandatory.length }}</b></button>
      <button type="button" role="tab" :aria-selected="tab === 'other'" :class="{ active: tab === 'other' }" @click="selectTab('other')">Басқа шығындар <b>{{ oneTime.length + ledgerOther.length }}</b></button>
    </div>

    <section v-if="tab === 'mandatory'" class="panel table-panel exp-panel">
      <header class="table-toolbar">
        <div><span class="eyebrow">Ай сайынғы міндеттемелер</span><h2>Міндетті төлемдер</h2><p>Әр ай төленуі керек. «Ай сайын» белгісі барлары жаңа айға көшіріледі.</p></div>
        <div class="toolbar-actions">
          <label class="search-field"><Search :size="17" /><input v-model="search" placeholder="Атауы, категориясы немесе бөлімі" /></label>
          <button class="button secondary" type="button" @click="openSubscription"><Plus :size="17" /> Подписка қосу</button>
          <button class="button primary" type="button" @click="openForm()"><Plus :size="17" /> Төлем қосу</button>
        </div>
      </header>
      <div class="exp-filterbar">
        <div class="segmented pay-status" role="group" aria-label="Төлем күйі">
          <button type="button" :class="{ active: status === 'all' }" @click="status = 'all'">Барлығы <b>{{ mandatory.length }}</b></button>
          <button type="button" :class="{ active: status === 'unpaid' }" @click="status = 'unpaid'">Төленбеген <b>{{ unpaidCount }}</b></button>
          <button type="button" :class="{ active: status === 'paid' }" @click="status = 'paid'">Төленген <b>{{ mandatory.length - unpaidCount }}</b></button>
        </div>
        <button v-if="!selectionMode && mandatory.length" class="button subtle" type="button" @click="toggleAll"><CheckCircle2 :size="16" /> Таңдау</button>
      </div>
      <div v-if="selectionMode" class="selection-bar active">
        ...
        <div v-if="selected.length"><strong>{{ selected.length }} таңдалды</strong><button type="button" @click="setPaid(true)"><CheckCircle2 :size="15" /> Төленді</button><button type="button" @click="setPaid(false)"><Clock3 :size="15" /> Төленбеді</button><button type="button" class="danger" @click="removeSelected"><Trash2 :size="15" /> Өшіру</button></div>
        <span v-else>Қажетті төлемдерді белгілеңіз</span>
      </div>

      <div v-if="categoryGroups.length" class="exp-groups">
        <div v-for="group in categoryGroups" :key="group.name" class="exp-group">
          <div class="exp-group-head"><span>{{ group.name }}</span><b>{{ formatMoney(group.total) }}</b><small v-if="group.unpaid" class="owed">қалды {{ formatMoney(group.unpaid) }}</small><small v-else class="done">төленді</small></div>
          <div v-for="expense in group.rows" :key="expense.id" class="exp-row" :class="{ paid: expense.isPaid, selected: selectedIds.has(expense.id), selecting: selectionMode }">
            ...
            <div class="expense-name"><span><ReceiptText :size="18" /></span><div><strong>{{ expense.name }}</strong><small>{{ expense.departmentName || "Жалпы" }}</small></div></div>
            <span :class="expense.isRecurring ? 'recurring-tag' : 'once-tag'">{{ expense.isRecurring ? "Ай сайын" : "Осы ай ғана" }}</span>
            <strong class="pay-amount">{{ formatMoney(expense.amount) }}</strong>
            <button type="button" class="status-toggle pay-toggle" :class="{ paid: expense.isPaid }" :disabled="payroll.isPending('toggleExpensePaid', expense.id)" @click="payroll.mutate('toggleExpensePaid', { id: expense.id, isPaid: !expense.isPaid })"><i><Check v-if="expense.isPaid" :size="13" /></i>{{ expense.isPaid ? "Төленді" : "Төлеу" }}</button>
      ...
      <div v-else class="empty-state"><span><ReceiptText :size="25" /></span><strong>{{ mandatory.length ? "Сүзгі бойынша төлем жоқ" : "Міндетті төлемдер жоқ" }}</strong><p>Аренда, интернет немесе подписканы қосыңыз.</p><button class="button primary" type="button" @click="openForm()"><Plus :size="17" /> Төлем қосу</button></div>
    </section>

    <section v-else class="panel exp-panel">
      <header class="table-toolbar">
        <div><span class="eyebrow">Бір реттік</span><h2>Басқа шығындар</h2><p>Сатып алулар мен бір реттік шығындар — бірден жұмсалған болып есептеледі және келесі айға көшірілмейді.</p></div>
        <label class="search-field"><Search :size="17" /><input v-model="search" placeholder="Шығын атауын іздеу" /></label>
      </header>
      <form class="quick-add" @submit.prevent="addOneTime">
        <input v-model="quickName" aria-label="Не алынды" placeholder="Не алынды? Мысалы: кеңсе орындығы" maxlength="120" required>
        <div class="money-input"><input v-model.number="quickAmount" aria-label="Сома" type="number" min="1" step="1" placeholder="Сома" required><b>₸</b></div>
        <button class="button primary" :disabled="payroll.saving.value"><Plus :size="17" /> {{ payroll.saving.value ? "Қосылуда…" : "Қосу" }}</button>
      </form>
      <div v-if="oneTimeRows.length" class="registry-list">
        <article v-for="(expense, index) in oneTimeRows" :key="expense.id"><span class="registry-number">{{ String(index + 1).padStart(2, "0") }}</span><div><strong>{{ expense.name }}</strong><small>Басқа шығындар · жұмсалды</small></div><b>{{ formatMoney(expense.amount) }}</b><span class="row-actions"><button type="button" aria-label="Өзгерту" @click="openOneTime(expense)"><Pencil :size="16" /></button><button type="button" class="danger" aria-label="Өшіру" @click="remove(expense)"><Trash2 :size="16" /></button></span></article>
      </div>
      <div v-else-if="!ledgerOther.length" class="empty-state compact"><span><ShoppingBag :size="25" /></span><strong>Бұл айда бір реттік шығын жоқ</strong><p>Жоғарыдағы жолға атауы мен сомасын жазып, «Қосу» басыңыз.</p></div>
      <div v-if="ledgerOther.length" class="ledger-block">
        <div class="exp-group-head"><span>Есепке енгізілген басқа шығындар</span><b>{{ formatMoney(ledgerOtherTotal) }}</b><NuxtLink :to="reportLink('other')" class="text-button">Есепте өзгерту <ArrowUpRight :size="13" /></NuxtLink></div>
        <div v-for="e in ledgerOther" :key="e.id" class="ledger-row"><div><strong>{{ e.name }}</strong><small>{{ FINANCE_CATEGORIES[e.category] }} · {{ e.origin === "import" ? "Excel импорт" : "Қолмен" }}</small></div><b>{{ formatMoney(e.amount ?? 0) }}</b></div>
      </div>
    </section>
  </div>
