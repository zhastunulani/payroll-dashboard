...
const filtered = computed(() => {
  ...
  return query ? rows.value.filter(item => `${item.name} ${item.categoryName} ${item.departmentName || ""}`.toLocaleLowerCase("kk-KZ").includes(query)) : rows.value;
});
...
function openForm(expense: ExpenseRecord | null = null) { editExpense.value = expense; subscriptionMode.value = false; formOpen.value = true; }
function openSubscription() { editExpense.value = null; subscriptionMode.value = true; formOpen.value = true; }
...
<template>
  <div class="page expenses-page">
    ...
    <section class="panel table-panel">
      ...
      <div v-if="filtered.length" class="expense-table responsive-table" :class="{ selecting: selectionMode }">
        <div class="table-header"><span v-if="selectionMode" /><span>Не алынды / шығын</span><span>Категория</span><span>Бөлім</span><span>Қайталану</span><span>Сома</span><span>Статус</span><span /></div>
        <div v-for="expense in filtered" :key="expense.id" class="table-record" :class="{ selected: selectedIds.has(expense.id) }">
          ...
          <div data-label="Бөлім"><strong>{{ expense.departmentName || "—" }}</strong></div>
  ...
  <ExpenseForm v-if="formOpen" :expense="editExpense" :subscription="subscriptionMode" @close="formOpen = false" @saved="formOpen = false" />
</template>
